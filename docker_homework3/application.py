...
def load_films(filename=settings.data_file_path):
    with open(filename) as f:
        return json.load(f)

def save_films(films_data, filename=settings.data_file_path):
...
def add_movie(film: Film):
    ...
    if len(films) >= settings.MAX_FILMS:
        return {"error": f"Cannot add movie. Maximum number of films ({settings.MAX_FILMS}) reached"}
